...
export function Testimonials() {
  return (
    <section
      id="testimonials"
      className="bg-sage-300 text-forest-950 py-24 md:py-36"
    >
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <Reveal className="max-w-3xl">
          ...
          <h2 className="font-display mt-8 text-5xl md:text-6xl leading-[0.95] text-forest-950">
            Their <em className="italic text-forest-700">words</em>, not mine.
          </h2>
        </Reveal>

        <RevealStagger className="mt-16 md:mt-20 grid md:grid-cols-3 gap-px bg-forest-950/10">
          {TESTIMONIALS.map((t) => (
            <RevealItem key={t.name}>
              <figure className="bg-sage-300 p-8 md:p-10 h-full flex flex-col">
                <Quote className="text-forest-600" size={28} strokeWidth={1.25} />
                <blockquote className="font-display italic mt-6 text-xl md:text-2xl leading-snug text-forest-950 flex-1">
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                <figcaption className="mt-8 pt-5 border-t border-forest-950/15">
                  <div className="text-sm text-forest-950">{t.name}</div>
        ...
        <p className="mt-8 text-[10px] uppercase tracking-widest-xl text-forest-800/30">

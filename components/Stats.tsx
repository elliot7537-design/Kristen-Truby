...
function StatItem({ stat, start }: { stat: Stat; start: boolean }) {
  ...
  return (
    <div className="py-10 md:py-0 md:px-8 border-t md:border-t-0 md:border-l border-cream-50/15 first:border-t-0 first:border-l-0">
      <div className="font-display text-6xl md:text-[110px] leading-none text-cream-50">
        {padded}
        <span className="text-sage-300">{stat.suffix ?? ""}</span>
      </div>
      <div className="mt-4 text-[11px] uppercase tracking-widest-xl text-cream-50/60">
        {stat.label}
      </div>
    </div>
  );
...
export function Stats() {
  ...
  return (
    <section className="bg-forest-500 text-cream-50 py-20 md:py-28 relative overflow-hidden">
      <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div
          ref={ref}
          className="mt-12 md:mt-16 grid md:grid-cols-3 border-t border-cream-50/15 pt-8"
        >
          {STATS.map((s) => (
